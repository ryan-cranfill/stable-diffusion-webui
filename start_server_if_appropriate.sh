...
# @boot sleep 30; /home/ryan/stable-diffusion-webui/start_server_if_appropriate.sh

# Get the current time in 24 hour format.
current_time=$(date +%H%M)

# If the current time is between 5:15pm and 7am, start the server.
if [[ $current_time -ge 1700 ]] || [[ $current_time -lt 700 ]]; then
    # Start the server.
    echo "Starting server..."
    /home/ryan/.nvm/versions/node/v19.4.0/bin/node /home/ryan/.nvm/versions/node/v19.4.0/bin/pm2 start all
    echo "Server started via pm2."
  # Else, ensure the server is not running.
else
    # Stop the server.
    echo "Stopping server..."
    /home/ryan/.nvm/versions/node/v19.4.0/bin/node /home/ryan/.nvm/versions/node/v19.4.0/bin/pm2 stop all
